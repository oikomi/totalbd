...
type Agent struct {
	...
	AgentGUID                   string
	AgentVersion                string
	plugin                      *baidu_platform_go.NewrelicPlugin
	HTTPTimer                   metrics.Timer
	HTTPStatusCounters          map[int]metrics.Counter
	Tracer                      *Tracer
	CustomMetrics               []baidu_platform_go.IMetrica
...
func NewAgent() *Agent {
	agent := &Agent{
		...
		Tracer:                      nil,
		CustomMetrics:               make([]baidu_platform_go.IMetrica, 0),
	}
	return agent
}
...
type resettableComponent struct {
	baidu_platform_go.IComponent
	counters map[int]metrics.Counter
}

// baidu_platform_go.IComponent interface implementation
...
func (agent *Agent) AddCustomMetric(metric baidu_platform_go.IMetrica) {
